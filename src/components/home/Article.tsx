  ...
  return (
    <div className="columns-2 lg:columns-3">
      {datas?.articles.map((article, index) => (
        <div
          key={index}
          className="bg-white dark:bg-[#101010] mb-5 break shadow-lg rounded-lg overflow-hidden transition-transform transform hover:scale-105"
        >
          {/* Image Section */}
          ...
          <div className="p-4">
            <Link
              href={article.url}
              passHref
              className="font-bold text-lg text-blue-600 dark:text-[#03A9F4] hover:underline"
            >
              {article.title}
            </Link>
            <p className="text-sm text-gray-700 dark:text-[#fff] mt-2">
              {article.description}
            </p>
